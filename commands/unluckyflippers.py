...
def execute(parser, bot, user, args):
    ...
    # parse their choice
    if topwhat == 1:
        topPlayer = bot.execQuerySelectOne("SELECT * FROM users WHERE (coins_won+coins_lost) > 0 ORDER BY (coins_lost - coins_won) DESC, (coins_won + coins_lost) ASC LIMIT 1")
        ...
        topPlayers = bot.execQuerySelectMultiple("SELECT * FROM users WHERE (coins_won+coins_lost) > 0 ORDER BY (coins_lost - coins_won) DESC, (coins_won + coins_lost) ASC LIMIT %d" % topwhat)
